...
@Component({
  selector: 'app-credential-check-table',
  templateUrl: './credential-check-table.component.html',
  styleUrls: ['./credential-check-table.component.scss']
})
export class CredentialCheckTableComponent implements OnInit {
  @Input() someDataCred: Array<BackgroundCandidate>;
  ...
  ngOnChanges(): void {
    console.log(`Im being called from the data-table: `, this.someDataCred);
    this.loading = true;
    this.ref.detectChanges();
    if(this.someDataCred) {
      this.httpService.getCandidateDataCredential(this.someDataCred).then((result: any[]) =>{
        if(result.length > 0) {
